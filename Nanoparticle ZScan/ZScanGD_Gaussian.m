...
% This is the approach originally used by Sheik-Bahae.
%
% Reference:
...
n2 = 2e-4; % Non-linear refractive index
lambda = 532e-9; % Green 
...
dm = k0*w0^2/2;
wm = w0*sqrt(1^2 + z2^2/dm^2);
Rm = z2/(1-1/(1^2 + z2^2/dm^2));
thetam = atan2(z2/dm, 1); 
EReplay_NoSample =  (1^2 + z2^2/dm^2)^(-1/2) .* exp( - rreplay.^2/wm^2 - 1i*k0*rreplay.^2/2/Rm + 1i*thetam);
S = sum(rreplay(rreplay<ra).*abs(EReplay_NoSample(rreplay<ra)).^2) / ...
    sum(rreplay.*abs(EReplay_NoSample).^2);
T0 = sum(rreplay(rreplay<ra).*abs(EReplay_NoSample(rreplay<ra)).^2);
